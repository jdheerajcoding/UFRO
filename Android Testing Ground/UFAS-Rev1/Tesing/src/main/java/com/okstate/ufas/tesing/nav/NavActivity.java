package com.okstate.ufas.tesing.nav;
...
public class NavActivity extends FragmentActivity {
	...
	protected void onResume() {
		...
		if (Sec.VerifyDeveloperID(Sec.GetDeviceID(getApplicationContext()), Sec.TestersIDs) || Sec.Verify(getApplicationContext(), "D..3")) {
			...
			dlgAlert.setPositiveButton(getString(R.string.Yes), new OnClickListener() {
	...
	public void SaveMissionOnClick(View v) {
		...
		alert.setPositiveButton(getString(R.string.Save), new OnClickListener() {
	...
	private final void DisplayInfoDialog(String title, String text, String buttonText) {
		...
		builder.setTitle(title).setMessage(text).setCancelable(false).setNegativeButton(buttonText, new OnClickListener() {
