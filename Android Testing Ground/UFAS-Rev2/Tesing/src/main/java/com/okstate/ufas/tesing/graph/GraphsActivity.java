...
import com.okstate.ufas.tesing.graph.GraphView.LegendAlign;
import com.okstate.ufas.tesing.graph.GraphViewSeries.GraphViewSeriesStyle;
			...
			if (!pause) {
				...
				if (CurentPosition == NextLimit) {
					for (GraphViewSeries s : series) {
						s.resetData(new GraphView.GraphViewData[] { new GraphView.GraphViewData(CurentPosition, 0) });
					}
				...
				for (GraphViewSeries s : series) {
					...
					if (s.description.equals(app.ACCROLL))
						s.appendData(new GraphView.GraphViewData(CurentPosition, app.mw.ax), true);
