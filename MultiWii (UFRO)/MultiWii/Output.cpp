...
void mixTable() {
  ...
  /****************                   main Mix Table                ******************/
    ...
    servo[3]  = get_middle(3) - (axisPID[YAW]*SERVODIR(3,2)) + (axisPID[PITCH]*SERVODIR(3,1)) - (axisPID[ROLL] *SERVODIR(3,1));   //   SideServo  5  D12 (right vane)  
    servo[4]  = get_middle(4) - (axisPID[YAW]*SERVODIR(4,2)) + (axisPID[PITCH]*SERVODIR(4,1)) - (axisPID[ROLL] *SERVODIR(4,1));     //   SideServo  3  D11 (left vane)
    servo[5]  = get_middle(5) - (axisPID[YAW]*SERVODIR(5,2)) + (axisPID[ROLL] *SERVODIR(5,1)) + (axisPID[PITCH]*SERVODIR(5,1));//   FrontServo 2  D3
    servo[6]  = get_middle(6) - (axisPID[YAW]*SERVODIR(6,2)) + (axisPID[ROLL] *SERVODIR(6,1)) + (axisPID[PITCH]*SERVODIR(6,1)); //   RearServo  4  D10
//    for(i=3;i<7;i++) {
//     servo[i]  = (axisPID[YAW] * SERVODIR(i,2)) + (axisPID[(6-i)>>1] * SERVODIR(i,1));  // mix and setup direction
//     servo[i] += get_middle(i);
//    }
    motor[0] = rcCommand[THROTTLE];  //was 0 it works at zero 
 
// #elif defined( SINGLECOPTER-mod )
//    /***************************          Modify 1         ******************************/
//    // Unmanned Flying and Rolling Orb 
//    // This is a beta version 
//    // Assisted modes (gyro only or gyro+acc according to AUX configuration in Gui
//    //  ben just to put get_middle code infront of each line. 
//    servo[3]  = get_middle(3)+(axisPID[YAW]*SERVODIR(3,2)) + (axisPID[PITCH]*SERVODIR(3,1));   //   SideServo  5  D12
//    servo[4]  = get_middle(3)+(axisPID[YAW]*SERVODIR(4,2)) + (axisPID[PITCH]*SERVODIR(4,1));   //   SideServo  3  D11
//    servo[5]  = get_middle(3)+(axisPID[YAW]*SERVODIR(5,2)) + (axisPID[ROLL] *SERVODIR(5,1));   //   FrontServo 2  D3
//    servo[6]  = get_middle(3)+(axisPID[YAW]*SERVODIR(6,2)) + (axisPID[ROLL] *SERVODIR(6,1));   //   RearServo  4  D10
//    88..............................................................
////    for(i=3;i<7;i++) {
////     servo[i]  = (axisPID[YAW] * SERVODIR(i,2)) + (axisPID[(6-i)>>1] * SERVODIR(i,1));  // mix and setup direction
////     servo[i] += get_middle(i);
//
//// how the hell they distingusih the roll and pitch ????
////    }
